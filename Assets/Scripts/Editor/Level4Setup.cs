...
namespace TrafficTown2D.Editor
{
    public static class Level4Setup
    {
        ...
        private static readonly Color BikeLaneColor = new Color(0.28f, 0.72f, 0.38f, 0.45f);
        ...
        public static void SetupLevel4()
        {
            ...
            EnsureCamera(); EnsureGlobalLight(); EnsureEventSystem();
            ...
            // Environment
            GameObject env = FindOrCreate("Environment");
            CreateRoadWithBikeLane(env.transform);

            // Traffic
            GameObject traffic = FindOrCreate("Traffic");
            CreateTrafficLane(traffic.transform);
            CreateCyclistLane(traffic.transform);

            // Player Car
            GameObject playerCar = CreatePlayerCar();
            CarPlayerController carCtrl = GetOrAdd<CarPlayerController>(playerCar);
            ...
            BikeLaneController bikeCtrl = GetOrAdd<BikeLaneController>(playerCar);
            SetReference(bikeCtrl, "playerCar", carCtrl);

            // UI
            LevelUIController ui = CreateUI(score, sceneLoader);
            FeedbackController feedbackCtrl = Object.FindAnyObjectByType<FeedbackController>(FindObjectsInactive.Include);
            SetReference(bikeCtrl, "feedback", feedbackCtrl);
            SetReference(bikeCtrl, "levelUI", ui);
            SetReference(bikeCtrl, "scoreManager", score);
            ...
            Debug.Log("Level 4 (Bike Lane Awareness) setup completed.");
        }

        private static void CreateRoadWithBikeLane(Transform parent)
        {
            ...
            CreateSprite(bg.transform, "BuildingB", new Vector3(0f, 5.3f, 2f), new Vector3(4f, 3f, 1f), new Color(0.90f, 0.84f, 0.78f, 1f), -5, false);
            CreateSprite(bg.transform, "BuildingC", new Vector3(6f, 4.8f, 2f), new Vector3(3.2f, 2.2f, 1f), new Color(0.78f, 0.88f, 0.82f, 1f), -5, false);

            GameObject road = FindOrCreateChild(parent, "Road");
            CreateSprite(road.transform, "RoadBase", Vector3.zero, new Vector3(22f, 4.4f, 1f), RoadColor, 0, false);
            CreateSprite(road.transform, "TopEdge", new Vector3(0f, 2.15f, -0.01f), new Vector3(22f, 0.1f, 1f), RoadEdgeColor, 1, false);
            CreateSprite(road.transform, "BottomEdge", new Vector3(0f, -2.15f, -0.01f), new Vector3(22f, 0.1f, 1f), RoadEdgeColor, 1, false);

            // Dashed center line
            for (float x = -10f; x <= 10f; x += 1.2f)
                CreateSprite(road.transform, "Dash_" + x.ToString("F1"), new Vector3(x, 0f, -0.01f), new Vector3(0.7f, 0.08f, 1f), LaneMarkColor, 1, false);

            // Bike lane (green strip on top lane)
            CreateSprite(road.transform, "BikeLane", new Vector3(0f, 1.6f, -0.02f), new Vector3(22f, 0.9f, 1f), BikeLaneColor, 2, false);
            CreateSprite(road.transform, "BikeIcon", new Vector3(-5f, 1.6f, -0.03f), new Vector3(0.4f, 0.4f, 1f), new Color(0.15f, 0.55f, 0.25f, 1f), 3, true);
            CreateSprite(road.transform, "BikeIcon2", new Vector3(3f, 1.6f, -0.03f), new Vector3(0.4f, 0.4f, 1f), new Color(0.15f, 0.55f, 0.25f, 1f), 3, true);

            // Sidewalks
            CreateSprite(road.transform, "SidewalkTop", new Vector3(0f, 3.25f, 0f), new Vector3(22f, 1.9f, 1f), SidewalkColor, -2, false);
            CreateSprite(road.transform, "SidewalkBottom", new Vector3(0f, -3.25f, 0f), new Vector3(22f, 1.9f, 1f), SidewalkColor, -2, false);

            // Bike lane sign
            CreateBikeLaneSign(parent, new Vector3(-7f, 2.6f, 0f));
        }

        private static void CreateBikeLaneSign(Transform parent, Vector3 position)
        {
            GameObject sign = FindOrCreateChild(parent, "BikeLaneSign");
            sign.transform.localPosition = position;
            CreateSprite(sign.transform, "Post", new Vector3(0f, -0.5f, 0f), new Vector3(0.08f, 1f, 1f), new Color(0.29f, 0.31f, 0.32f, 1f), 12, false);
            CreateSprite(sign.transform, "Face", new Vector3(0f, 0.2f, 0f), new Vector3(0.7f, 0.55f, 1f), new Color(0.15f, 0.55f, 0.25f, 1f), 13, false);
        }

        private static void CreateTrafficLane(Transform parent)
        {
            ...
            GameObject lane = FindOrCreateChild(parent, "TopTrafficLane");
            Transform spawn = FindOrCreateChild(lane.transform, "SpawnPoint").transform;
            spawn.position = new Vector3(-9.5f, 0.8f, 0f);
            Transform stop = FindOrCreateChild(lane.transform, "StopPoint").transform;
            stop.position = new Vector3(-2.8f, 0.8f, 0f);
            Transform exit = FindOrCreateChild(lane.transform, "ExitPoint").transform;
            exit.position = new Vector3(9.5f, 0.8f, 0f);
            ...
            SetFloat(spawner, "travelDirection", 1f);
        }

        private static void CreateCyclistLane(Transform parent)
        {
            GameObject lane = FindOrCreateChild(parent, "CyclistLane");

            // Create a cyclist prefab-like object
            for (int i = 0; i < 2; i++)
            {
                float startX = -8f - i * 5f;
                GameObject cyclist = new GameObject("Cyclist_" + i);
                cyclist.transform.SetParent(lane.transform, false);
                cyclist.transform.position = new Vector3(startX, 1.6f, 0f);

                Rigidbody2D body = cyclist.AddComponent<Rigidbody2D>();
                body.gravityScale = 0f;
                body.freezeRotation = true;

                BoxCollider2D col = cyclist.AddComponent<BoxCollider2D>();
                col.size = new Vector2(0.6f, 0.5f);

                CyclistAI ai = cyclist.AddComponent<CyclistAI>();
                ai.Configure(1.5f, 12f, 1f);

                CreateSprite(cyclist.transform, "Body", Vector3.zero, new Vector3(0.5f, 0.7f, 1f), new Color(0.3f, 0.7f, 0.4f, 1f), 5, false);
                CreateSprite(cyclist.transform, "Wheel", new Vector3(0f, -0.3f, 0f), new Vector3(0.3f, 0.3f, 1f), new Color(0.2f, 0.2f, 0.2f, 1f), 4, true);
            }
        }
        ...
        private static void CreateIntroPanel(LevelUIController ui)
        {
            ...
            TextMeshProUGUI titleTxt = CreateUIText(card.transform, "Title", "BIKE LANE AWARENESS", 24, TextAlignmentOptions.Center, new Vector2(400f, 36f), new Vector2(0f, 110f), new Vector2(0.5f, 0.5f));
            TextMeshProUGUI msgTxt = CreateUIText(card.transform, "Message", "Drive with A/D keys.\n\nPress Q or E to check blind spots.\nDo NOT hit cyclists!", 16, TextAlignmentOptions.Center, new Vector2(380f, 120f), new Vector2(0f, 10f), new Vector2(0.5f, 0.5f));
        ...
        // ─── UI Creation (mirrors Level3Setup) ──────────────────────────────────
        private static LevelUIController CreateUI(ScoreManager score, SceneLoader sceneLoader)
        {
            ...
            TextMeshProUGUI objective = CreateUIText(missionCard.transform, "Objective", "Check blind spots before crossing the bike lane!", 14, TextAlignmentOptions.Left, new Vector2(290f, 32f), new Vector2(10f, -10f), new Vector2(0f, 0.5f));
            ...
            CreateUIText(completionCard.transform, "Subtitle", "Great bike lane awareness!", 17, TextAlignmentOptions.Center, new Vector2(420f, 28f), new Vector2(0f, 134f), new Vector2(0.5f, 0.5f));
        ...
        private static GameObject FindOrCreate(string name) { GameObject go = GameObject.Find(name); return go != null ? go : new GameObject(name); }
        ...
        private static T GetOrAdd<T>(GameObject go) where T : Component { if (!go.TryGetComponent<T>(out var c)) c = go.AddComponent<T>(); return c; }

        private static GameObject CreateSprite(Transform parent, string name, Vector3 pos, Vector3 scale, Color color, int order, bool circle)
        {
        ...
        private static Sprite EnsureSprite(string path, bool circle)
        {
            ...
            Color32[] px = new Color32[sz * sz]; Color32 w = new Color32(255,255,255,255); Color32 cl = new Color32(255,255,255,0);
            float c = (sz-1)*0.5f; float rSq = (sz*0.46f)*(sz*0.46f);
            for (int y=0;y<sz;y++) for (int x=0;x<sz;x++) { float dx=x-c,dy=y-c; px[y*sz+x]=(!circle||dx*dx+dy*dy<=rSq)?w:cl; }
            tex.SetPixels32(px); tex.Apply(); File.WriteAllBytes(path, tex.EncodeToPNG());
            AssetDatabase.ImportAsset(path); return AssetDatabase.LoadAssetAtPath<Sprite>(path);
        }

        // UI helpers
        private static GameObject CreateUIPanel(Transform parent, string name, Color color, Sprite sprite = null)
        { GameObject go = FindOrCreateChild(parent, name); EnsureRectTransform(go); Image img = GetOrAdd<Image>(go); img.sprite = sprite; img.type = sprite != null ? Image.Type.Sliced : Image.Type.Simple; img.color = color; return go; }

        private static TextMeshProUGUI CreateUIText(Transform parent, string name, string content, float fontSize, TextAlignmentOptions align, Vector2 size, Vector2 pos, Vector2 anchor)
        { GameObject go = FindOrCreateChild(parent, name); RectTransform r = EnsureRectTransform(go); SetRect(r, anchor, anchor, pos, size); TextMeshProUGUI t = GetOrAdd<TextMeshProUGUI>(go); t.text = content; t.fontSize = fontSize; t.alignment = align; t.color = Color.white; return t; }

        private static TextMeshProUGUI CreateStatCard(Transform parent, string name, string label, string value, Vector2 pos, Sprite sprite)
        { GameObject card = CreateUIPanel(parent, name, new Color(0.92f, 0.94f, 0.96f, 1f), sprite); SetRect(card.GetComponent<RectTransform>(), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), pos, new Vector2(180f, 76f));
          ...
          TextMeshProUGUI val = CreateUIText(card.transform, "Value", value, 24, TextAlignmentOptions.Left, new Vector2(140f, 32f), new Vector2(12f, -10f), new Vector2(0.5f, 0.5f)); val.fontStyle = FontStyles.Bold; return val; }

        private static Button GetOrAddChildButton(Transform parent, string name, string label, Vector2 pos, Sprite sprite)
        { GameObject go = CreateUIPanel(parent, name, new Color(0.12f, 0.55f, 0.84f, 1f), sprite); SetRect(go.GetComponent<RectTransform>(), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), pos, new Vector2(250f, 60f));
          Button btn = GetOrAdd<Button>(go); GetOrAdd<AnimatedUIButton>(go);
          TextMeshProUGUI t = CreateUIText(go.transform, "Text", label, 18, TextAlignmentOptions.Center, new Vector2(230f, 40f), Vector2.zero, new Vector2(0.5f, 0.5f)); t.fontStyle = FontStyles.Bold; return btn; }

        private static RectTransform EnsureRectTransform(GameObject go) { if (go.TryGetComponent<RectTransform>(out var rt)) return rt; return go.AddComponent<RectTransform>(); }
        private static void SetRect(RectTransform rt, Vector2 anchorMin, Vector2 anchorMax, Vector2 pos, Vector2 size) { rt.anchorMin = anchorMin; rt.anchorMax = anchorMax; rt.anchoredPosition = pos; rt.sizeDelta = size; }

        private static void SetReference(Object target, string field, Object value)
        { SerializedObject so = new SerializedObject(target); SerializedProperty sp = so.FindProperty(field); if (sp != null) { sp.objectReferenceValue = value; so.ApplyModifiedPropertiesWithoutUndo(); } }
        private static void SetObjectArray(Object target, string field, Object[] values)
        { SerializedObject so = new SerializedObject(target); SerializedProperty sp = so.FindProperty(field); if (sp == null) return; sp.ClearArray(); for (int i = 0; i < values.Length; i++) { sp.InsertArrayElementAtIndex(i); sp.GetArrayElementAtIndex(i).objectReferenceValue = values[i]; } so.ApplyModifiedPropertiesWithoutUndo(); }
        private static void SetFloat(Object target, string field, float value) { SerializedObject so = new SerializedObject(target); SerializedProperty sp = so.FindProperty(field); if (sp != null) { sp.floatValue = value; so.ApplyModifiedPropertiesWithoutUndo(); } }
        private static void SetInt(Object target, string field, int value) { SerializedObject so = new SerializedObject(target); SerializedProperty sp = so.FindProperty(field); if (sp != null) { sp.intValue = value; so.ApplyModifiedPropertiesWithoutUndo(); } }
    }
}
#endif
